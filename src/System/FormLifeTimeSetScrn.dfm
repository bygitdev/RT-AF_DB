object FormLifeTimeSet: TFormLifeTimeSet
  Left = 0
  Top = 0
  BorderStyle = bsNone
  ClientHeight = 780
  ClientWidth = 1280
  Color = clGray
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object PanelSystemBase: TPanel
    Left = 0
    Top = 0
    Width = 1280
    Height = 780
    Align = alClient
    Color = clGray
    ParentBackground = False
    TabOrder = 0
    object CurvyPanel1: TCurvyPanel
      Left = 5
      Top = 5
      Width = 1270
      Height = 770
      TabOrder = 0
      object BtnParamIndexMoveM10: TSpeedButton
        Tag = -10
        Left = 1154
        Top = 50
        Width = 93
        Height = 114
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Calibri'
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        ParentFont = False
      end
      object BtnParamIndexMoveP10: TSpeedButton
        Tag = 10
        Left = 1154
        Top = 568
        Width = 93
        Height = 114
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Calibri'
        Font.Style = []
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        ParentFont = False
      end
      object BtnSaveItem: TSpeedButton
        Tag = 1
        Left = 999
        Top = 698
        Width = 130
        Height = 40
        Caption = 'Save Data'
        Enabled = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = #47569#51008' '#44256#46357
        Font.Style = [fsBold]
        Glyph.Data = {
          
          3232323232323232323232FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        ParentFont = False
        OnClick = BtnSaveItemClick
      end
      object BtnInputItem: TSpeedButton
        Tag = 1
        Left = 863
        Top = 698
        Width = 130
        Height = 40
        Caption = 'Input Data'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = #47569#51008' '#44256#46357
        Font.Style = [fsBold]
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        ParentFont = False
        OnClick = BtnInputItemClick
      end
      object Label1: TLabel
        Left = 28
        Top = 18
        Width = 117
        Height = 26
        Caption = 'LIFE TIME SET'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -21
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
      end
      object SpeedButton1: TSpeedButton
        Tag = 1
        Left = 28
        Top = 698
        Width = 153
        Height = 40
        Caption = 'Lifetime Reset'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = #47569#51008' '#44256#46357
        Font.Style = [fsBold]
        ParentFont = False
        OnClick = SpeedButton1Click
      end
      object LifeTimeParamGrid: TStringGrid
        Left = 28
        Top = 50
        Width = 1101
        Height = 632
        DefaultRowHeight = 32
        RowCount = 31
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnDrawCell = LifeTimeParamGridDrawCell
        ColWidths = (
          64
          561
          150
          149
          150)
      end
    end
  end
  object LifeTimeTimer: TTimer
    Enabled = False
    OnTimer = LifeTimeTimerTimer
    Left = 173
    Top = 26
  end
end
