object FormSecsGemSet: TFormSecsGemSet
  Left = 0
  Top = 0
  BorderStyle = bsNone
  ClientHeight = 780
  ClientWidth = 1280
  Color = clGray
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object CurvyPanel1: TCurvyPanel
    Left = 8
    Top = 8
    Width = 1266
    Height = 766
    Color = clBtnFace
    TabOrder = 0
    object BtnSaveLog: TSpeedButton
      Tag = 1
      Left = 17
      Top = 711
      Width = 146
      Height = 42
      Caption = 'SAVE LOG'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = [fsBold]
      Glyph.Data = {
        
        3232323232323232323232FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      OnClick = BtnSaveLogClick
    end
    object SecsMemo: TMemo
      Left = 19
      Top = 295
      Width = 662
      Height = 410
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = #47569#51008' '#44256#46357
      Font.Style = []
      ImeName = 'Microsoft IME 2003'
      ParentFont = False
      ReadOnly = True
      ScrollBars = ssVertical
      TabOrder = 0
      OnChange = SecsMemoChange
    end
    object BtnClose: TBitBtn
      Left = 1103
      Top = 711
      Width = 146
      Height = 42
      Caption = 'CLOSE'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Calibri'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ParentFont = False
      TabOrder = 1
      OnClick = BtnCloseClick
    end
    object GroupBox1: TGroupBox
      Left = 17
      Top = 17
      Width = 504
      Height = 133
      Caption = 'SECS/GEM STATUS'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = []
      ParentBackground = False
      ParentColor = False
      ParentFont = False
      TabOrder = 2
      object Label1: TLabel
        Left = 24
        Top = 29
        Width = 93
        Height = 18
        Caption = 'PROCESS STATE'
      end
      object Label2: TLabel
        Left = 24
        Top = 61
        Width = 150
        Height = 18
        Caption = 'COMMUNICATION STATE'
      end
      object Label3: TLabel
        Left = 24
        Top = 93
        Width = 97
        Height = 18
        Caption = 'CONTROL STATE'
      end
      object PanelGemState: TPanel
        Left = 216
        Top = 18
        Width = 273
        Height = 38
        BevelInner = bvRaised
        BevelKind = bkSoft
        BevelOuter = bvLowered
        Caption = 'SETUP'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -19
        Font.Name = 'Calibri'
        Font.Style = []
        ParentBackground = False
        ParentFont = False
        TabOrder = 0
      end
      object EditCommState: TPanel
        Left = 216
        Top = 54
        Width = 273
        Height = 38
        BevelInner = bvRaised
        BevelKind = bkSoft
        BevelOuter = bvLowered
        Caption = 'SETUP'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -19
        Font.Name = 'Calibri'
        Font.Style = []
        ParentBackground = False
        ParentFont = False
        TabOrder = 1
      end
      object EditCtrlState: TPanel
        Left = 216
        Top = 91
        Width = 273
        Height = 38
        BevelInner = bvRaised
        BevelKind = bkSoft
        BevelOuter = bvLowered
        Caption = 'SETUP'
        Color = clWhite
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -19
        Font.Name = 'Calibri'
        Font.Style = []
        ParentBackground = False
        ParentFont = False
        TabOrder = 2
      end
    end
    object GroupboxSecsControl: TGroupBox
      Left = 535
      Top = 17
      Width = 351
      Height = 133
      Caption = 'SECS/GEM CONTROL'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = []
      ParentBackground = False
      ParentColor = False
      ParentFont = False
      TabOrder = 3
      object BtnSecsCommStop: TSpeedButton
        Tag = 1
        Left = 196
        Top = 78
        Width = 139
        Height = 45
        Caption = 'STOP'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
        OnClick = BtnSecsCommStopClick
      end
      object BtnSecsCommStart: TSpeedButton
        Tag = 1
        Left = 196
        Top = 23
        Width = 139
        Height = 45
        Caption = 'START'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
        OnClick = BtnSecsCommStartClick
      end
      object BtnSecsAutoStart: TSpeedButton
        Tag = 1
        Left = 16
        Top = 47
        Width = 157
        Height = 45
        Caption = 'AUTO START ON'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
        OnClick = BtnSecsCommStartClick
      end
    end
    object GroupBox2: TGroupBox
      Left = 17
      Top = 156
      Width = 1232
      Height = 133
      Caption = 'DATA CONTROL'
      Color = clBtnFace
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = []
      ParentBackground = False
      ParentColor = False
      ParentFont = False
      TabOrder = 4
      object GroupBox3: TGroupBox
        Left = 16
        Top = 25
        Width = 289
        Height = 97
        Caption = 'PORT INFO'
        TabOrder = 0
        object EditOutPortID: TEdit
          Left = 9
          Top = 57
          Width = 130
          Height = 34
          Alignment = taCenter
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -21
          Font.Name = 'Calibri'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 0
          Text = '0'
        end
        object EditPortID: TEdit
          Left = 9
          Top = 20
          Width = 130
          Height = 34
          Alignment = taCenter
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -21
          Font.Name = 'Calibri'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 1
          Text = '0'
        end
        object BtnGemPortID: TButton
          Left = 157
          Top = 24
          Width = 124
          Height = 27
          Caption = 'PORT ID'
          TabOrder = 2
          OnClick = BtnGemPortIDClick
        end
        object BtnGemOutPortID: TButton
          Left = 157
          Top = 61
          Width = 124
          Height = 27
          Caption = 'OUTPORT ID'
          TabOrder = 3
          OnClick = BtnGemOutPortIDClick
        end
      end
      object EditOPid: TEdit
        Left = 311
        Top = 52
        Width = 105
        Height = 34
        Alignment = taCenter
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -21
        Font.Name = 'Calibri'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        Text = '0'
      end
      object ComboBoxCarrierType: TComboBox
        Left = 311
        Top = 17
        Width = 105
        Height = 27
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = [fsBold]
        ItemIndex = 0
        ParentFont = False
        TabOrder = 2
        Text = 'FOUP'
        Items.Strings = (
          'FOUP'
          'FOSB'
          'MAGAZINE'
          'MAC'
          'TRAY'
          'CASSETTE')
      end
      object BtnGemCarrierType: TButton
        Left = 422
        Top = 13
        Width = 106
        Height = 34
        Caption = 'CARRIER TYPE'
        TabOrder = 3
        OnClick = BtnGemCarrierTypeClick
      end
      object BtnGemOPID: TButton
        Left = 422
        Top = 52
        Width = 106
        Height = 34
        Caption = 'OPERATOR ID'
        TabOrder = 4
        OnClick = BtnGemOPIDClick
      end
      object BtnBOATID: TButton
        Left = 422
        Top = 93
        Width = 106
        Height = 34
        Caption = 'BOAT ID'
        TabOrder = 5
        OnClick = BtnBOATIDClick
      end
      object EditMapSENDBoatID: TEdit
        Left = 311
        Top = 92
        Width = 105
        Height = 34
        Alignment = taCenter
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -21
        Font.Name = 'Calibri'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 6
        Text = '0'
      end
      object eXGem: TeXGem
        Left = 534
        Top = 14
        Width = 32
        Height = 32
        OneSECSMessageReceived = eXGemeSECSMessageReceived
        OneGEMCommStateChanged = eXGemeGEMCommStateChanged
        OneGEMControlStateChanged = eXGemeGEMControlStateChanged
        OneGEMErrorEvent = eXGemeGEMErrorEvent
        OneGEMReqRemoteCommand = eXGemeGEMReqRemoteCommand
        OneGEMReqPPLoadInquire = eXGemeGEMReqPPLoadInquire
        OneGEMRspPPLoadInquire = eXGemeGEMRspPPLoadInquire
        OneGEMTerminalMessage = eXGemeGEMTerminalMessage
        OneXGEMStateEvent = eXGemeXGEMStateEvent
        OneGEMReqPPSendEx = eXGemeGEMReqPPSendEx
        OneGEMRspPPEx = eXGemeGEMRspPPEx
        OneGEMReqPPEx = eXGemeGEMReqPPEx
        OneGEMReqOnline = eXGemeGEMReqOnline
        ControlData = {00000100560A00002B05000000000000}
      end
      object Button1: TButton
        Left = 1110
        Top = 95
        Width = 119
        Height = 34
        Caption = 'Make Recipe File'
        TabOrder = 8
        OnClick = Button1Click
      end
      object Button2: TButton
        Left = 1110
        Top = 55
        Width = 119
        Height = 34
        Caption = 'Rcp File Upload'
        TabOrder = 9
        OnClick = Button2Click
      end
      object Button3: TButton
        Tag = 1
        Left = 998
        Top = 96
        Width = 106
        Height = 34
        Caption = 'Make Trace File'
        TabOrder = 10
        OnClick = Button1Click
      end
      object Button4: TButton
        Tag = 1
        Left = 998
        Top = 56
        Width = 106
        Height = 34
        Caption = 'Tra File Upload'
        TabOrder = 11
        OnClick = Button2Click
      end
      object edtModuleID: TEdit
        Left = 844
        Top = 103
        Width = 149
        Height = 27
        Alignment = taCenter
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 12
        Text = 'RECIPENAME'
      end
      object Button5: TButton
        Left = 663
        Top = 17
        Width = 75
        Height = 25
        Caption = 'Button5'
        TabOrder = 13
        OnClick = Button5Click
      end
      object FEcmrTime: TEdit
        Left = 999
        Top = 16
        Width = 105
        Height = 34
        Alignment = taCenter
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -21
        Font.Name = 'Calibri'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 14
        Text = '0'
      end
      object ReqTime: TButton
        Left = 1110
        Top = 15
        Width = 119
        Height = 34
        Caption = 'REQ TIME(F/ECMR)'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
        TabOrder = 15
        OnClick = ReqTimeClick
      end
    end
    object gbSecsgemOn: TGroupBox
      Left = 887
      Top = 26
      Width = 362
      Height = 124
      Color = clBtnFace
      ParentBackground = False
      ParentColor = False
      TabOrder = 5
      object BtnCommEnable: TSpeedButton
        Tag = 1
        Left = 5
        Top = 14
        Width = 171
        Height = 45
        Caption = 'COMMUNICATION'#13#10'ENABLE'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
        OnClick = BtnCommEnableClick
      end
      object BtnOnlineRemote: TSpeedButton
        Tag = 1
        Left = 187
        Top = 14
        Width = 171
        Height = 45
        Caption = #8594' ONLINE REMOTE'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
        OnClick = BtnOnlineRemoteClick
      end
      object BtnOnlineLocal: TSpeedButton
        Tag = 1
        Left = 187
        Top = 69
        Width = 171
        Height = 45
        Caption = #8592' ONLINE LOCAL'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
        OnClick = BtnOnlineLocalClick
      end
      object BtnOffline: TSpeedButton
        Tag = 1
        Left = 5
        Top = 69
        Width = 171
        Height = 45
        Caption = #9632' OFFLINE'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Calibri'
        Font.Style = []
        ParentFont = False
        OnClick = BtnOfflineClick
      end
    end
    object PageControl1: TPageControl
      Left = 680
      Top = 295
      Width = 569
      Height = 250
      ActivePage = TabSheet5
      TabOrder = 6
      object TabSheet1: TTabSheet
        Caption = 'Process State Changed'
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton1: TSpeedButton
          Left = 3
          Top = 185
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton1Click
        end
        object Memo1: TMemo
          Left = 3
          Top = 5
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 1001'
            '[SVID]'
            '(2001) EQUIPMENT ID = '
            '(1001) PROCESS STATE = ')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet2: TTabSheet
        Caption = 'Recipe Upload Req'
        ImageIndex = 1
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton2: TSpeedButton
          Left = 3
          Top = 186
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton2Click
        end
        object Memo2: TMemo
          Left = 3
          Top = 5
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 1201'
            '[SVID] - 7'
            '(2001) EQUIPMENT ID = '
            '(2006) LOT ID = '
            '(2025) PART ID ='
            '(2092) RECIPE ID ='
            '(2024) STEP ='
            '(2093) LOTTYPE =')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet3: TTabSheet
        Caption = 'Recipe Download Req'
        ImageIndex = 2
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton3: TSpeedButton
          Tag = 1
          Left = 3
          Top = 186
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton2Click
        end
        object Memo3: TMemo
          Left = 3
          Top = 5
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 1202'
            '[SVID] - 7'
            '(2001) EQUIPMENT ID = '
            '(2006) LOT ID = '
            '(2025) PART ID ='
            '(2092) RECIPE ID ='
            '(2024) STEP ='
            '(2093) LOTTYPE =')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet4: TTabSheet
        Caption = 'LOT DISPLAY'
        ImageIndex = 4
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton4: TSpeedButton
          Left = 3
          Top = 186
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton4Click
        end
        object Memo4: TMemo
          Left = 3
          Top = 5
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 2004'
            '[SVID] - 2'
            '(2011) EQUIPMENT ID = '
            '(2006) LOT ID =')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet5: TTabSheet
        Caption = 'VERSION UPDATE'
        ImageIndex = 5
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton5: TSpeedButton
          Left = 3
          Top = 186
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton5Click
        end
        object Memo5: TMemo
          Left = 3
          Top = 5
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 2021'
            '[SVID] - 3'
            '(2001) EQUIPMENT ID = '
            '(2034) EQP MODEL = '
            '(2035) OS VERSION = ')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet6: TTabSheet
        Caption = 'LOT END(OTHER)'
        ImageIndex = 6
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton6: TSpeedButton
          Left = 3
          Top = 186
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton6Click
        end
        object Memo6: TMemo
          Left = 3
          Top = 5
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 2101'
            '[SVID] - 7'
            '(2001) EQUIPMENT ID = '
            '(2006) LOT ID ='
            '(2004) OPER ID ='
            '(2039) EWS LOT = ""'
            '(2066) SBL CHECK RESULT = ""'
            '(2038) AUTOLINE = "NO"'
            '(2013) PORT ID =  ex) AROUT-X07_O1')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet12: TTabSheet
        Caption = 'CHANGE MAZID'
        ImageIndex = 11
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton12: TSpeedButton
          Left = 2
          Top = 183
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton12Click
        end
        object Memo12: TMemo
          Left = 2
          Top = 2
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 2005'
            '[SVID] - 4'
            '(2001) EQUIPMENT ID = '
            '(2006) LOT ID = '
            '(2012) MGZ ID = '
            '(2004) OPER ID = ')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet7: TTabSheet
        Caption = 'ADD SCRAP'
        ImageIndex = 7
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton7: TSpeedButton
          Left = 3
          Top = 186
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton7Click
        end
        object Memo7: TMemo
          Left = 3
          Top = 5
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 2114'
            '[SVID] - 7'
            '(2001) EQUIPMENT ID = '
            '(2006) LOT ID = '
            '(2004) OPER ID = '
            '(2038) AUTOLINE = NO'
            '(2046) SCRAP CNT = 2'
            
              '(2047) SCRAP INFO = 3305:1M393A4K40BB2XXXA05,3404:1M393A4K40BB2X' +
              'XXA0_2'
            '(2024) STEP = '
            '')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet10: TTabSheet
        Caption = 'SCRAP INFO'
        ImageIndex = 9
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton10: TSpeedButton
          Left = 3
          Top = 185
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton10Click
        end
        object Memo10: TMemo
          Left = 3
          Top = 3
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 2115'
            '[SVID] - 4'
            '(2001) EQUIPMENT ID = '
            '(2006) LOT ID = '
            '(2004) OPER ID = '
            '(2038) AUTOLINE = NO')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet11: TTabSheet
        Caption = 'DELETE SCRAP'
        ImageIndex = 10
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton11: TSpeedButton
          Left = 3
          Top = 185
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton11Click
        end
        object Memo11: TMemo
          Left = 3
          Top = 4
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 2116'
            '[SVID] - 4'
            '(2001) EQUIPMENT ID = '
            '(2006) LOT ID = '
            '(2067) DELETE SERIAL = '
            '(2038) AUTOLINE = NO')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet8: TTabSheet
        Caption = 'EQ EVENT'
        ImageIndex = 8
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton8: TSpeedButton
          Left = 3
          Top = 186
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton8Click
        end
        object Memo8: TMemo
          Left = 3
          Top = 3
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            'CEID = 9000 ~ 9011'
            '- Input 7'
            '(CEID) EVENT NO = 9000 ~ 9011'
            '(2001) EQUIPMENT ID = '
            '(2006) LOT ID = '
            '(2025) PART ID = '
            '(2024) STEP = '
            '(2018) QTY = '
            '(9000) STAGE NO = ')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
      object TabSheet9: TTabSheet
        Caption = 'FREE'
        ImageIndex = 8
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object SpeedButton9: TSpeedButton
          Left = 3
          Top = 185
          Width = 555
          Height = 34
          Caption = 'SEND CEID'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          OnClick = SpeedButton9Click
        end
        object Memo9: TMemo
          Left = 3
          Top = 3
          Width = 555
          Height = 177
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Tahoma'
          Font.Style = []
          Lines.Strings = (
            '- Input 8, From Row 2, Col 2 SVID'
            '(CEID) EVENT NO'
            '(VID1)'
            '(VID2)'
            '(VID3)'
            '(VID4)'
            '...'
            '(VID7)')
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
        end
      end
    end
    object StringGrid1: TStringGrid
      Left = 680
      Top = 547
      Width = 567
      Height = 158
      BorderStyle = bsNone
      ColCount = 3
      FixedCols = 0
      RowCount = 10
      Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goEditing]
      TabOrder = 7
      ColWidths = (
        64
        414
        64)
    end
  end
  object TMOnlineRemote: TTimer
    Enabled = False
    OnTimer = TMOnlineRemoteTimer
    Left = 464
    Top = 80
  end
  object GemOnlineTimer: TTimer
    Enabled = False
    Interval = 2000
    OnTimer = GemOnlineTimerTimer
    Left = 552
    Top = 80
  end
  object SendVersionTimer: TTimer
    Interval = 7000
    OnTimer = SendVersionTimerTimer
    Left = 463
    Top = 137
  end
  object tmRcpUpload: TTimer
    Enabled = False
    OnTimer = tmRcpUploadTimer
    Left = 633
    Top = 252
  end
end
