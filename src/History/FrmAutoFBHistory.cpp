...
void CalculateAverages(CppSQLite3DB& db, const string& tableName, const vector<string>& columns, int nBridgeCnt, int nStageNo, int nAgvCnt)
{
	try
	{
		MainForm->MachineHistoryData("AutoFeedBack -> CalculateAverages");
		...
		MainForm->MachineHistoryData("AutoFeedBack -> qcInfoSend");
...
void  TFormAutoFBHistory::AutoFBAgv(int nBridgeCnt, int nStageNo)
{
	try
	{
		MainForm->MachineHistoryData("Start AutoFBAgv!!");
...
void __fastcall TFormAutoFBHistory::Btn_DatabaseReadClick(TObject *Sender)
{
	...
	AnsiString strStartDateTime = DateTimeAutoFeedBackStart->DateTime.FormatString("yyyy-mm-dd HH:nn:ss");
	AnsiString strEndDateTime = DateTimeAutoFeedBackEnd->DateTime.FormatString("yyyy-mm-dd HH:nn:ss");
	AnsiString dbPath = ExtractFilePath(Application->ExeName) + "Ini\\system\\DB\\AutoFeedBackHistory.DB";


	// 백그라운드 스레드로 실행
	TReadDBThread* thread = new TReadDBThread(dbPath, strStartDateTime, strEndDateTime, "AutoFeedBackHistory", "QcTime");
	thread->Start();
...
void TFormAutoFBHistory::Record_AutoFeedBack(AUTOFEEDBACK_DATA data)
{
	...
	try
	{
		MainForm->MachineHistoryData( "Record_AutoFeedBack Try!!" );
...
void  TFormAutoFBHistory::AutoFBDataSave()
{
	try
	{
		...
		{
			MainForm->MachineHistoryData("Start AutoFBDataSave!!");
